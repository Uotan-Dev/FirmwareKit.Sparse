...
public class SparseFileIntegrationTests
{
    ...
    [Fact]
    public void CountChunks_ReturnsSameAsManualCalculation()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize * 5);
        // create gaps: one raw, one fill, then a gap
        sparseFile.AddRawChunk(new byte[BlockSize]);           // blocks 0..0
        sparseFile.AddFillChunk(0x12345678, BlockSize, blockIndex: 2); // blocks 2..2
        // total blocks set to 5 -> trailing gap of 2 blocks

        // manual: raw chunk, implicit skip chunk for index 1, fill chunk, trailing skip
        Assert.Equal(4u, sparseFile.CountChunks());
    }

    [Fact]
    public async Task AsyncReadWrite_IsFunctionallyEquivalent()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize * 3);
        sparseFile.AddRawChunk(new byte[BlockSize]);
        sparseFile.AddFillChunk(0xABCDEF01, BlockSize);

        using var mem = new MemoryStream();
        await SparseWriter.WriteToStreamAsync(sparseFile, mem, sparse: true, gzip: true, includeCrc: true);
        mem.Position = 0;
        var parsed = await SparseReader.FromStreamAsync(mem, validateCrc: true);
        Assert.Equal(sparseFile.Header.TotalBlocks, parsed.Header.TotalBlocks);
        Assert.Equal(sparseFile.Chunks.Count, parsed.Chunks.Count);
    }

    [Fact]
    public void WriteToStream_WithGzip_ProducesGzipHeader()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize);
        sparseFile.AddDontCareChunk(BlockSize);

        using var ms = new MemoryStream();
        sparseFile.WriteToStream(ms, sparse: true, gzip: true);
        var data = ms.ToArray();
    ...
    [Fact]
    public void GetLength_MatchesWrittenSizes()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize * 4);
        sparseFile.AddRawChunk(new byte[BlockSize]);
        sparseFile.AddDontCareChunk(BlockSize);
        long lenSparse = sparseFile.GetLength(sparse: true, includeCrc: false);
        long lenRaw = sparseFile.GetLength(sparse: false, includeCrc: false);

        using var ms = new MemoryStream();
        sparseFile.WriteToStream(ms, sparse: true);
        Assert.Equal(lenSparse, ms.Length);

        using var mr = new MemoryStream();
        sparseFile.WriteRawToStream(mr);
        Assert.Equal(lenRaw, mr.Length);
    }

    [Fact]
    public void WriteWithCallback_NonSparse_CreatesExpectedBytes()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize * 2);
        sparseFile.AddDontCareChunk(BlockSize);
        using var expected = new MemoryStream();
        sparseFile.WriteRawToStream(expected);

        using var cb = new MemoryStream();
        sparseFile.WriteWithCallback((b,l) => { if(b!=null) cb.Write(b,0,l); return 0; }, sparse: false);
        Assert.Equal(expected.ToArray(), cb.ToArray());
    }

    [Fact]
    public void AddStreamChunk_WorksWithMemoryStream()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize);
        var buffer = Enumerable.Range(0, (int)BlockSize).Select(i => (byte)i).ToArray();
        using var ms = new MemoryStream(buffer);
        sparseFile.AddStreamChunk(ms, 0, BlockSize);
        Assert.Single(sparseFile.Chunks);
        Assert.Equal(BlockSize, sparseFile.Chunks[0].DataProvider.Length);
    }

    [Fact]
    public void GetLength_IncludesCrcWhenRequested()
    {
        using var sparseFile = new SparseFile(BlockSize, BlockSize);
        sparseFile.AddRawChunk(new byte[BlockSize]);
        long noCrc = sparseFile.GetLength(sparse: true, includeCrc: false);
        long withCrc = sparseFile.GetLength(sparse: true, includeCrc: true);
        Assert.True(withCrc > noCrc);
    }
